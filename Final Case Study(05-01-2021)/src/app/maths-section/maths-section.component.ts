...
@Component({
  selector: 'app-maths-section',
  templateUrl: './maths-section.component.html',
  styleUrls: ['./maths-section.component.css']
})
export class MathsSectionComponent implements OnInit {

  constructor(private mathsService:MathsServiceService,
  ...
  books=[];
    getData(){
      this.mathsService.getDetails().subscribe((data)=>{
        this.allBooks=data;
        console.log(this.allBooks);
        for(let book of this.allBooks){
          if(book.sectionId==25){
  ...
  section=[];
  getSection(){
    this.mathsService.getSection().subscribe(data =>{
  ...
  borrow(book){
    ...
    // if( available==1){
      if(count<=2){
        if(flag==0){
          alert('Book Borrowed');
          this.mathsService.updateQuantity(this.updateQuantity).subscribe(()=>{
            // this.getData();
          });
          this.mathsService.postBorrower(this.borrowerDetails).subscribe(()=>{
            this.getBorrowerDetails();
          });
        }else
        {
...
details;
  getBorrowerDetails(){
    this.mathsService.getBorrowerData().subscribe((data)=>{
      this.details=data;
